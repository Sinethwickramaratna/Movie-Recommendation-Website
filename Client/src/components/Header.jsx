...
export default function Header() {
    ...
    useEffect(() => {
        function handleClickOutside(event) {
            // Check if click is outside search ref AND outside mobile overlays
            const isOutsideSearch = searchRef.current && !searchRef.current.contains(event.target);
            const isOutsideMobileOverlays = !event.target.closest('#mobile-overlays');

            if (isOutsideSearch && isOutsideMobileOverlays) {
        ...
        <header className="sticky top-0 z-50 w-full border-b border-white/10 bg-background-dark/70 backdrop-blur-xl animate-fade-in">
            <div className="mx-auto flex h-16 sm:h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-2 sm:gap-4 relative" ref={searchRef}>
                    ...
                    <button type="button" onClick={() => { setIsMobileSearchOpen(!isMobileSearchOpen); setIsMobileMenuOpen(false); }} className="flex lg:hidden h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full text-slate-300 hover:text-white hover:bg-white/10 active:bg-white/20 transition-colors cursor-pointer">
                        <span className="material-symbols-outlined text-2xl sm:text-3xl pointer-events-none">search</span>
                    </button>
                    <button type="button" onClick={() => { setIsMobileMenuOpen(!isMobileMenuOpen); setIsMobileSearchOpen(false); }} className="md:hidden flex h-10 w-10 sm:h-11 sm:w-11 items-center justify-center rounded-full text-white hover:bg-white/10 active:bg-white/20 transition-colors sm:ml-1 cursor-pointer">
                        <span className="material-symbols-outlined text-2xl sm:text-3xl pointer-events-none">{isMobileMenuOpen ? 'close' : 'menu'}</span>
                    </button>
                </div>
            </div>

            {/* Mobile Overlays Container */}
            <div id="mobile-overlays">
